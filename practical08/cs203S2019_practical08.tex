...
\newcommand{\mainprogram}{\lstinline{compute_tf_plugins.py}}
\newcommand{\mainprogramsource}{\lstinline{termfrequency/compute_tf_plugins.py}}

\newcommand{\secondprogram}{\lstinline{plugins-compile-and-install.sh}}
\newcommand{\secondprogramsource}{\lstinline{plugins-src/plugins-compile-and-install.sh}}

\newcommand{\testprogram}{\lstinline{test_compute_tf_plugins.py}}
\newcommand{\testprogramsource}{\lstinline{tests/test_compute_tf_plugins.py}}
...
\newcommand{\gitcommitmainprogram}{\command{git commit termfrequency/compute_tf_plugins.py -m "Descriptive commit message"}}
\newcommand{\gitcommittestprogram}{\command{git commit tests/test_compute_tf_plugins.py -m "Descriptive commit message"}}
...
development programs such as a ``terminal window'' and a ``plugin compiler''.
You will continue to practice using Slack to support communication with the
...
completing the project. You will improve the source code of a Python program
implemented in the plugin-oriented style and observe how the plugins enable you
to dynamically change the program's behavior through a configuration file.
...
\item {\bf Explore teamwork and technologies}. While certain plugins of these
...
through 19 in the {\em Exercises in Programming Style\/}, focusing on the
content about the plugin style.
%
Make sure that you review the relevant content in \cooperative~and
\philosophy.
%
You are also expected to find and read all of the online resources that you need
to complete this assignment.

\section*{Creating and Running a Plugin-Based Python Program}
...
like
``Allegheny-Computer-Science-203-Spring-2019/computer-science-203-spring-2019-practical-8-gkapfham''.
...
%
As you are working to understand all of the provided code, you should review the
notes and markers in it so that you can enhance a Python program and a shell
script in the correct fashion.
%
Make sure that you notice that this Python program contains ``plugins'' that are
``installed'' into the main program. What are the benefits associated with using
the plugin style to dynamically install functionality into the program?
%
Don't forget that, before you can run this program, you must compile and install
the four plugins on which it relies, following the completed source code after
this paragraph that must run from a provided shell script.
...
Once you have correctly installed and configured the plugins through the
\program{config.ini} file, you should try to run the program in all four of its
possible configurations. Does it work correctly? Make sure that you understand
how the Python program finds and invokes its plugins. Students are encouraged to
draw a technical diagram showing how each step of the plugin style works.
%
You should also ensure that you understand why it is necessary to run each of
the steps in the script before running \mainprogramsource{} with
\command{pipenv} and \command{python3}. Don't forget that if you modify a
plugin's code you must re-compile and re-install it before running the program!
...
\item Revised versions of \mainprogramsource{} and \secondprogramsource{} that,
  when appropriate, contains refactored and improved source code.

\item Stored in a Markdown file called \reflection{}, a three-paragraph response,
  with each paragraph consisting of at least 50 words, for each of the stated
